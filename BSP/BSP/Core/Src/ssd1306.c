...
const uint8_t INIT_SSD1306[]  = {
  ...
  // 0xA8
  1, SSD1306_DISPLAY_CLOCK_DIV, 0x80, //mio
...
//  2, SSD1306_SET_PAGE_ADDR, START_PAGE_ADDR, END_PAGE_ADDR,
  ...
  1, SSD1306_SET_PRECHARGE, 0xf1, // MIO 0xc2,
  // Set V COMH Deselect, reset value 0x22 = 0,77xUcc
  1, SSD1306_VCOM_DESELECT, 0x40, //MIO 0x20,
  // 0xA4
...
{ 
  ...
  while (no_of_commands) {
    ...
    while (no_of_arguments--) {
      // send command
      status = SSD1306_Send_Command (*commands++);
      // request - start TWI
      if (SSD1306_SUCCESS != status) {
        // error
        return status;
...
uint8_t SSD1306_UpdateScreen (uint8_t address)
{
  ...
  {
	  status = SSD1306_Send_StartAndSLAW (address);
	  status = SSD1306_Send_Command (SSD1106_PAGE_SELECT | (j>>7));
	  status = SSD1306_Send_Command (0x00);
	  status = SSD1306_Send_Command (0x10);
	  TWI_Stop ();
	  ...
	  for (i=0;i<128;i++)
	  {
		status = TWI_MT_Send_Data (cacheMemLcd[i+j]);
		// request succesfull
		if (SSD1306_SUCCESS != status) {
		  // error
		  return status;
...
struct I2CData{
	uint8_t address;
	uint8_t mode;
	uint8_t data[CACHE_SIZE_MEM+2];
	uint16_t count;
	HAL_StatusTypeDef status;
};

struct I2CData I2C_Transfer;
...
/**
  ...
  * @desc    TWI Send SLAW
  * @param   void
  * @return  unsigned uint8_t
  */
 uint8_t TWI_MT_Send_SLAW(uint8_t address)
 {
	 I2C_Transfer.mode=0;
	 I2C_Transfer.address=address;
	 return SUCCESS;
 }
 ...
 {
	 I2C_Transfer.data[I2C_Transfer.count++]=data;
	 return SUCCESS;
 }
 ...
 {
	 HAL_StatusTypeDef status;
	 if (I2C_Transfer.mode==1)
	 {
		 status=HAL_I2C_Master_Receive(&hi2c3,I2C_Transfer.address<<1,I2C_Transfer.data,I2C_Transfer.count,10000);
	 }
	 else
	 {
		 status=HAL_I2C_Master_Transmit(&hi2c3,I2C_Transfer.address<<1,I2C_Transfer.data,I2C_Transfer.count,10000);
	 }
	 I2C_Transfer.status=status;
 }
